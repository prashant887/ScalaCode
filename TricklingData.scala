...
import org.apache.spark.sql.functions.{when,count,countDistinct}
...
val TransactionData=spark.read.format("csv").schema(DataSchema)
  ...
  .load("D:\\transactions-sample.csv")
    ...
    TransactionData.select($"Id",$"chain").distinct().show(truncate = false)
    TransactionData.select(count($"Id"),countDistinct($"Id")).show(truncate = false)
  }
}
